...
FOR EACH ROW
BEGIN
    INSERT INTO log (action, table_name, changed_data, timestamp)
    VALUES ('create', 'menu_items', CONCAT(NEW.menu_item_id, ':', NEW.name), NOW());
END $$
...
BEGIN
    INSERT INTO log (action, table_name, changed_data, timestamp)
    VALUES ('update', 'menu_items', CONCAT(NEW.menu_item_id, ':', NEW.name), NOW());
END $$
...
BEGIN
    INSERT INTO log (action, table_name, changed_data, timestamp)
    VALUES ('delete', 'menu_items', CONCAT(OLD.menu_item_id, ':', OLD.name), NOW());
END $$
